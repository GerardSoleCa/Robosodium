...
public class Hash {
    ...
    private static final int OPS_LIMIT = 524288;
    private static final int MEM_LIMIT = 16777216;
    ...
    public byte[] pwhash_scryptsalsa208sha256(String passwd, byte[] salt, int opslimit, long memlimit) {
        buffer = new byte[KEY_LEN];
        sodium().crypto_pwhash_scryptsalsa208sha256(buffer, buffer.length, passwd.getBytes(), passwd.getBytes().length, salt, OPS_LIMIT, MEM_LIMIT);
        return buffer;
    }

    public String pwhash_scryptsalsa208sha256(String passwd, Encoder encoder, byte[] salt, int opslimit, long memlimit) {
        buffer = new byte[KEY_LEN];
        sodium().crypto_pwhash_scryptsalsa208sha256(buffer, buffer.length, passwd.getBytes(), passwd.getBytes().length, salt, opslimit, memlimit);
        return encoder.encode(buffer);
    }


    public byte[] blake2(byte[] message, byte[] key) throws UnsupportedOperationException {
        if (!blakeSupportedVersion()) throw new UnsupportedOperationException();

        buffer = new byte[BLAKE2B_OUTBYTES];
        sodium().crypto_generichash_blake2b(buffer, BLAKE2B_OUTBYTES, message, message.length, key, key.length);
        return buffer;
    }

    public String blake2(String message, Encoder encoder) throws UnsupportedOperationException {
        if (!blakeSupportedVersion()) throw new UnsupportedOperationException();
        byte[] hash = blake2(message.getBytes(), null);
        return encoder.encode(hash);
    }
